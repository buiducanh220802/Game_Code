...
Item::Item(int x, int y, TileType type, SDL_Texture* texture)
	: x(x), y(y), type(type), texture(texture) {}

void Item::render(SDL_Renderer* renderer, int offsetX, int offsetY) {
	SDL_Rect  dst = {
		x * TILE_SIZE + offsetX,
		y * TILE_SIZE + offsetY,
		TILE_SIZE, TILE_SIZE
